...
const PROMPT = 'a commercial 1/7 scale figurine of the main object in the picture was created, depicting a realistic style and a realistic environment. The figurine is placed on a computer desk with a round transparent acrylic base. There is no text on the base. The computer screen shows the Zbrush modeling process of the figurine. Next to the computer screen is a BANDAI-style toy box with the original painting printed on it.'
...
export default {
  ...
  async run(criv, { m }) {
    ...
    if (!/image/.test(mime)) return criv.reply(m, '📸 Kirim atau reply gambar dulu!')
      ...
      await criv.sendFile(m.chat, hasil, 'figur.png', '✨ Selesai!', m)
