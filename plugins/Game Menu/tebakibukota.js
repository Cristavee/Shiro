...
export default {
  ...
  async run(criv, { m, sender }) {
    ...
    try {
      const { data } = await axios.get('https://api.vreden.my.id/api/v1/game/ibukota')
      const soal = data?.result
      if (!soal?.jawaban || !soal?.soal) {
        console.log('[DEBUG] API response:', data)
        return m.reply('Soal tidak lengkap dari API. Jawaban atau pertanyaan tidak ditemukan.')
      }
