...
async function up(buffer) {
  ...
  try {
    ...
    if (res.error) throw new Error('Upload gagal: ' + res.error.message)

    const url = res.files?.[0]?.url
    if (!url) throw new Error('Respons upload tidak valid.')

    return url
  } catch (err) {
    if (err.response) throw new Error(`Upload gagal dengan status ${err.response.status}: ${err.response.statusText}`)
    else if (err.request) throw new Error('Tidak ada respons dari server Uguu.se.')
    else throw new Error('Error saat mengunggah: ' + err.message)
  }
}
