...
export default {
  ...
  tag: 'search',
  description: 'Mencari video di TikTok berdasarkan kata kunci.',
  owner: false,
  admin: false,
  botAdmin: false,
  public: true,
  ...
  async run(criv, { m, text }) {
    if (!text) {
      return m.reply('Masukkan kata kunci untuk mencari video TikTok.\n\nContoh: .tiktoksearch phonk');
    }

    try {
      const apiUrl = `https://api.diioffc.web.id/api/search/tiktok?query=${encodeURIComponent(text)}`;
      const { data } = await axios.get(apiUrl);

      const results = data?.result;

      if (!Array.isArray(results) || results.length === 0) {
        return m.reply(`Video TikTok untuk "${text}" tidak ditemukan. Coba kata kunci lain.`);
      }
      ...
      console.error(err);
      return m.reply('Terjadi kesalahan saat mencari video TikTok. Mohon coba lagi nanti.');
    }
  }
};
