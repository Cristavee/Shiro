...
function isSpamming(senderId, command) {
  ...
  const now = Date.now()
  const spamData = spamMap.get(key) || { count: 0, lastTime: 0 }
...
if (!m.quoted && m.message) {
  const type = Object.keys(m.message).find(v =>
    ['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(v)
  )
  if (type) {
...
const isOwner = db.data.owner.includes(sender.split('@')[0])
  ...
  try {
    if (typeof content === 'string') {
      ...
      await criv.sendMessage(from, { sticker: content, ...messageOptions }, { quoted: m });
    ...
    try {
      ...
      if (fromJid === sess.sender) {
        ...
        if (m.isMedia && mediaBuffer) {
          if (isImage) await criv.sendMessage(sess.recipient, { image: mediaBuffer, caption: label, ...sendOptions() })
          else if (isVideo) await criv.sendMessage(sess.recipient, { video: mediaBuffer, caption: label, ...sendOptions() })
          else if (isAudio) {
            if (label) await criv.sendMessage(sess.recipient, { text: label, ...sendOptions() })
            await criv.sendMessage(sess.recipient, { audio: mediaBuffer, mimetype: m.mimetype || 'audio/mpeg', ...sendOptions() })
          }
          else if (isDocument) await criv.sendMessage(sess.recipient, { document: mediaBuffer, mimetype: m.mimetype || 'application/octet-stream', fileName: 'file', caption: label, ...sendOptions() })
          else if (isSticker) {
            await criv.sendMessage(sess.recipient, { sticker: mediaBuffer, ...sendOptions() })
            if (label) await criv.sendMessage(sess.recipient, { text: label, ...sendOptions() })
          }
          else await criv.sendMessage(sess.recipient, { text: label, ...sendOptions() })
        } else {
          await criv.sendMessage(sess.recipient, { text: label, ...sendOptions() })
        }
      }
      ...
      // Message from recipient -> sender
        ...
        if (m.isMedia && mediaBuffer) {
          if (isImage) await criv.sendMessage(sess.sender, { image: mediaBuffer, caption: label, ...sendOptions() })
          else if (isVideo) await criv.sendMessage(sess.sender, { video: mediaBuffer, caption: label, ...sendOptions() })
          else if (isAudio) {
            if (label) await criv.sendMessage(sess.sender, { text: label, ...sendOptions() })
            await criv.sendMessage(sess.sender, { audio: mediaBuffer, mimetype: m.mimetype || 'audio/mpeg', ...sendOptions() })
          }
          else if (isDocument) await criv.sendMessage(sess.sender, { document: mediaBuffer, mimetype: m.mimetype || 'application/octet-stream', fileName: 'file', caption: label, ...sendOptions() })
          else if (isSticker) {
            await criv.sendMessage(sess.sender, { sticker: mediaBuffer, ...sendOptions() })
            if (label) await criv.sendMessage(sess.sender, { text: label, ...sendOptions() })
          }
          else await criv.sendMessage(sess.sender, { text: label, ...sendOptions() })
        } else {
          await criv.sendMessage(sess.sender, { text: label, ...sendOptions() })
        }
      }
    } catch (err) {
...
if (failReason) {
  ...
  return
        ...
        if (isStickerCommand && isMediaMessage && !m.quoted) {
            try {
                ...
                await plugin.run(criv, {
                    m, body, from, args, command, sender, db, pushName, text,
                    greet: global.getGreet(m.pushName), isBlocked,
        ...
        try {
          ...
          await plugin.run(criv, {
            m, body, from, args, command, sender, pushName, text, db,
            greet: global.getGreet(m.pushName), quoted, isBlocked,
            target, amount, user, helpers, mentioned, readMore, fakeQuote, getName,
            delay: global.delay,
            sleep: global.sleep,
