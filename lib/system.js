...
const system = {
  ...
  if (!db.data.users[id]) {
    db.data.users[id] = {
      ...
      status: null,
      gameStats: { totalPlayed: 0, totalWins: 0 }
    }
  ...
  getOwnerList() {
    const dbOwners = db.data.owner || []
    const globalOwners = global.owner || []
    return [...new Set([...dbOwners, ...globalOwners])]
  },
  ...
  getTier(rawId) {
    ...
    if (id && id === topUserId && topUserCoin >= 100000) {
      return { name: 'SULTAN', badge: '👑 SULTAN' }
    }
    if (coin >= 30000) return { name: 'Diamond', badge: '💎 Diamond' }
    if (coin >= 15000) return { name: 'Gold', badge: '🥇 Gold' }
    if (coin >= 5000) return { name: 'Silver', badge: '🥈 Silver' }
    if (coin >= 1000) return { name: 'Bronze', badge: '🥉 Bronze' }
    return { name: 'Basic', badge: '🪵 Basic' }
  },
  ...
  gameStats(rawId) {
    const id = sanitizeId(rawId)
    const user = system.getUser(id)
    user && (user.gameStats ||= { totalPlayed: 0, totalWins: 0 })
    return user?.gameStats || { totalPlayed: 0, totalWins: 0 }
  },
