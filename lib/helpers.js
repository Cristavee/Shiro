...
criv.sendFile = async (jid, input, URLmimetype = '', fileName = 'file', quoted = null, options = {}) => {
  let buffer = input

  // --- kalau input string (URL), auto download ---
  if (typeof input === 'string') {
    try {
      const res = await axios.get(input, { responseType: 'arraybuffer' })
      buffer = Buffer.from(res.data)
    } catch (e) {
      throw new Error(`Gagal download file dari URL: ${input}`)
    }
  }

  if (!Buffer.isBuffer(buffer)) throw new Error('sendFile: input harus Buffer atau URL string!')

  // --- cek mimetype otomatis ---
  const type = await fileTypeFromBuffer(buffer) || { mime: mimetype }
  const mime = type.mime || mimetype || 'application/octet-stream'

  // --- cek ukuran biar ga crash kalau file gede ---
  const MAX_SIZE = 20 * 1024 * 1024 // 20MB
  if (buffer.length > MAX_SIZE) {
    await criv.sendMessage(jid, {
      text: `⚠️ File terlalu besar (${(buffer.length/1024/1024).toFixed(2)}MB)\nTidak bisa dikirim langsung.\n\n🔗 Silakan unduh manual:\n${typeof input === 'string' ? input : 'File tidak ada link.'}`
    }, { quoted })
    return
  }
  ...
  if (mime.startsWith('image/')) {
    return criv.sendImage(jid, buffer, fileName, quoted, options)
  }
  if (mime.startsWith('video/')) {
    return criv.sendVideo(jid, buffer, fileName, quoted, options)
  }
  if (mime.startsWith('audio/')) {
    return criv.sendAudio(jid, buffer, false, quoted, options)
  }

  return criv.sendFormattedMessage(
    jid,
